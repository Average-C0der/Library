  ...
  function render() {
    ...
    for (let i = 0; i < myLibrary.length; i++) {
      if (myLibrary[i].read) {
        library.innerHTML +=
          ...
          '<span class="main">Title : </span><span class="book__title">' +
          ` ${myLibrary[i].title}` +
          "</span>" +
          "</div>" +
          '<div class="author__content">' +
          '<span class="main">Author : </span><span class="book__author">' +
          ` ${myLibrary[i].author}` +
          "</span>" +
          "</div>" +
          '<div class="pages__content">' +
          '<span class="main">Pages : </span><span class="book__pages">' +
          ` ${myLibrary[i].pages}` +
          "</span>" +
          "</div>" +
      ...
      function toggleBooks(e) {
        ...
        if (button.classList.contains("fa-check")) {
          if (
            button.parentNode.parentNode.firstChild.classList.contains("no")
          ) {
            button.parentNode.parentNode.firstChild.classList.toggle("yes");
            if (
              button.parentNode.parentNode.firstChild.textContent ===
              "I have not read it"
            ) {
    ...
    function filter(e) {
      ...
      if (e.target.classList.contains("all")) {
        ...
        books.forEach((book) => {
          var read = book
            .querySelector(".book__read-elements .book__read")
            .classList.contains("yes"); //get the read value from html
          if (read) {
            book.style.display = "flex";
          } else {
